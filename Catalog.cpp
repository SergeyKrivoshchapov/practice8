...
Smartphone Catalog::getSmartphone(string model) {
    transform(model.begin(), model.end(), model.begin(), [](unsigned char c) {return tolower(c); });
    for (Smartphone s : smartphones) {
        string currModel = s.getModel();
        transform( currModel.begin(), currModel.end(), currModel.begin(), [](unsigned char c) { return tolower(c); });
        if (currModel == model)
            return s;
    } return Smartphone();
}
